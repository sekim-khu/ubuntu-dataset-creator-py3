...
def get_random_utterances_from_corpus(candidate_dialog_paths,rng,utterances_num=9,min_turn=3,max_turn=20):
    ...
    """
    utterances = []
    dialogs_num = len(list(candidate_dialog_paths))
...
def create_examples(candidate_dialog_paths, creator_function, data_set):
    """
    ...
    :return:
    """
    for i in tqdm(range(len(candidate_dialog_paths))):
        context_dialog = candidate_dialog_paths[i]
        data_set.append(creator_function(context_dialog, candidate_dialog_paths))
...
if __name__ == '__main__':
    ...
    def create_eval_dataset(args, file_list_csv):
        ...
        dialog_paths = list(dialog_paths)
        
        manager = multiprocessing.Manager()
        data_set = manager.list()       
        
        procs = []
        len_in_proc = int(len(dialog_paths)/num_proc)
        for num in range(num_proc):
            if num == num_proc-1:
                proc = multiprocessing.Process(target=create_examples, 
                                    args=(dialog_paths[num*len_in_proc:],
                                    lambda context_dialog, candidates :
                                    create_single_dialog_test_example(context_dialog, candidates, rng,
                                                                       args.n, max_context_length=args.max_context_length),
                                    data_set))
            else:
                proc = multiprocessing.Process(target=create_examples, 
                                    args=(dialog_paths[num*len_in_proc:(num+1)*len_in_proc],
                                    lambda context_dialog, candidates :
                                    create_single_dialog_test_example(context_dialog, candidates, rng,
                                                                       args.n, max_context_length=args.max_context_length),
                                    data_set))
